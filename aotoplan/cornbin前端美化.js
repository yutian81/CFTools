...
function getIndexHtml(data, _clientOffset) {
  ...
  const tasksLists = taskKeys
   ...
   } autocomplete="false" >${key}</span>
   <span class="td">
     <input type="submit" formaction="/tasks/${key}/edit" style="visibility: hidden; display: none;">
     <input class="w-md" type="text" autocomplete="off" name="interval" value="${interval}" required placeholder="例如：*/5 * * * *">
   </span>
   <span class="td">
     <textarea rows="1" class="w-lg" name="url" autocomplete="off" required placeholder="请输入URL地址或cURL命令">${url}</textarea>
   </span>     
   <span class="td">
     <input class="mr mb" type="submit" formaction="/tasks/${key}/edit" value="保存">
     <button formaction="/tasks/${key}/run" class="mr mb">立即执行</button>
     <button formaction="/tasks/${key}/delete">删除</button>
   </span>
   <span class="td">
     <input class="w-md" value="${note}" autocomplete="off" type="text" name="note" placeholder="任务备注">
   </span>
   <span class="td">${logsHtml}</span>
    ...
    const body = `<main>
    <h2>定时任务管理系统</h2>
    <p>由 ❤️ <a href="https://www.owenyoung.com/">Owen</a> 开发 (<a href="https://github.com/theowenyoung/blog/tree/main/scripts/cronbin">源码地址</a>)</p>
  
  <div class="table">
  <div class="tr">
    <span class="td"><b>ID</b></span>
    <span class="td"><b>定时规则</b></span>
    <span class="td"><b>请求地址</b></span>
    <span class="td"><b>操作</b></span>
    <span class="td"><b>备注</b></span>
    <span class="td"><b>执行日志</b></span>
  </div>
  <form class="tr" autocomplete="off"  method="POST">
    <span class="td"></span>
    <span class="td"><input type="text" placeholder="例如：*/5 * * * *" name="interval" required></span>
    <span class="td"><textarea rows="1" placeholder="请输入URL地址或cURL命令" name="url" required></textarea></span>
    <span class="td"><button type="submit">添加任务</button></span>
    <span class="td"><input type="text" placeholder="任务说明" name="note"></span>
  </form>
  ${tasksLists}
  </div>
  ...
  <section>
    ...
    <form action="/notification" method="POST">
      <textarea rows="2" cols="80" 
        placeholder="请输入cURL通知命令（可使用 {{message}} 作为错误信息占位符）"
        name="notification_curl">${data.notification_curl || ""}</textarea>
      <br />
...
function logToText(log, _clientOffset) {
  ...
  message = message || "";
  message = message.replace('unknownError', '未知错误');
...
function timeToText(time, clientOffset) {
  ...
  // 新增完整日期格式汉化
  const clientDate = new Date(time.getTime() + clientOffset * 60 * 1000);
  return `${clientDate.getUTCFullYear()}-${addZero(clientDate.getUTCMonth() + 1)}-${addZero(clientDate.getUTCDate())} ${addZero(clientDate.getUTCHours())}:${addZero(clientDate.getUTCMinutes())}`;
}
